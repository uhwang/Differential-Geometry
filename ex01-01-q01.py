...
def movie_tcircle(t):
    ...
    dev.line(cx, cy, cx, tc, vgl.color.MAGENTA, dev.frm.hgt()*0.003)
    ...
    dev.circle(-rr, cy, r1, fcol=vgl.color.BLACK)
    dev.circle(px, py, r1, fcol=vgl.color.BLACK)
    dev.circle(cx, tc, r1, fcol=vgl.color.BLACK)
    
    #t_val.set_text(cx, tc -r1 if tc <=0 else r1, "%1.2f"%tc)
    #vgl.write_text(dev, t_val)
...
def save_tcircle_mov():
    global dev, tcircle_trail_x, tcircle_trail_y
    dev = vgl.DeviceCairo("", fmm.get_gbbox(), 300)
    dev.set_device(frm)
    dev_mov = vgl.DeviceCairoAnimation("ex01-01-q01.mp4", dev, movie_tcircle, dur, fps)
    dev_mov.save_video()
    
    tcircle_trail_x =[]
    tcircle_trail_y =[]
    dev = vgl.DeviceCairo("", fmm.get_gbbox(), 100)
    dev.set_device(frm)
    dev_mov = vgl.DeviceCairoAnimation("ex01-01-q01.gif", dev, movie_tcircle, dur, fps)
    dev_mov.save_gif()
